...
function parseTrainingJson(raw: unknown): TrainingContent[] {
  try {
    ...
    if (typeof parsed === "object" && parsed !== null)
      return [parsed as TrainingContent];
  } catch {
    // ignore parse errors
  }
  return [];
...
function StudyGuideRenderer({
  ...
  const diagramsBySection = new Map<
    string,
    typeof media extends undefined
      ? never
      : NonNullable<TrainingMedia["diagrams"]>
  >();
  const imagesBySection = new Map<
    string,
    NonNullable<TrainingMedia["images"]>
  >();
  const videosBySection = new Map<
    string,
    NonNullable<TrainingMedia["videos"]>
  >();
  ...
  const renderMediaForSection = (heading: string) => {
    ...
    return (
      <div key={`media-${key}`}>
        {images.map((img) => (
          <MediaImageCard key={img.id} image={img} />
        ))}
        {diagrams.map((d) => (
          <MermaidDiagram key={d.id} diagram={d} />
        ))}
        {videos.map((v) => (
          <VideoRecommendation key={v.id} video={v} />
        ))}
      </div>
    );
  };
    ...
    <div className="space-y-2 text-sm text-gray-800 leading-relaxed">
      {lines.map((line, i) => {
        if (line.startsWith("## ")) {
          ...
          return (
            <div key={i}>
              <h2 className="text-lg font-bold text-gray-900 mt-4 mb-1">
                {heading}
              </h2>
        ...
        if (line.startsWith("#### ")) {
          return (
            <h4 key={i} className="text-sm font-semibold text-gray-800 mt-2">
              {line.replace("#### ", "")}
            </h4>
          );
        }
...
function StatusBadge({ status }: { status: TrainingStatus }) {
  return (
    <span
      className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium border ${STATUS_STYLES[status] ?? STATUS_STYLES.Draft}`}
    >
      {status}
    </span>
  );
      ...
      {workingContents.length > 1 && (
        <div className="flex gap-2 flex-wrap">
          {workingContents.map((_c, i) => {
            const label =
              training.role === "Software Developer" && i === 0
                ? "Software Developer"
                : training.role === "Development Lead" && i === 1
                  ? "Development Lead"
                  : `Module ${i + 1}`;
            return (
              <button
      ...
      {studyMode === "guide" && (
        <>
          <StudyToolsPanel
            ...
            studyGuideMarkdown={content.study_guide}
          />
          <StudyGuideNarrator text={content.study_guide} />
          <div className="bg-white border border-gray-200 rounded-xl p-6">
            <StudyGuideRenderer
              markdown={content.study_guide}
              media={content.media}
            />
          </div>
        </>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <AssessmentRenderer
            ...
            onFlashcardQuestionsChange={
              onFlashcardQuestionsChange
                ? (questions) =>
                    onFlashcardQuestionsChange(contentIndex, questions)
                : undefined
            }
...
export function TrainingModulesPage() {
  ...
  const [flashcardSaveMsg, setFlashcardSaveMsg] = useState<{
    type: "ok" | "err";
    text: string;
  } | null>(null);
  ...
  const [acceptLoadingRevision, setAcceptLoadingRevision] = useState<
    number | null
  >(null);
  const [rejectLoadingRevision, setRejectLoadingRevision] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    ...
    const loadRevisions = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(body.detail || "Failed to load revisions.");
        }

        setRevisions((body.revisions ?? []) as TrainingRevision[]);
      } catch (err) {
        if (!mounted) return;
        setRevisions([]);
        setRevisionError(
  ...
  const handleFlashcardQuestionsChange = async (
    contentIndex: number,
    questions: Question[],
  ) => {
    if (!selected) return;
    ...
    setWorkingContents((prev) => {
      nextContents = prev.map((c, i) =>
        i === contentIndex
          ? { ...c, assessment: { ...c.assessment, questions } }
          : c,
      );
      return nextContents;
    });
    ...
    const trainingId =
      typeof selected.id === "string" ? Number(selected.id) : selected.id;
    ...
    setRows((prev) =>
      prev.map((r) =>
        r.id === selected.id ? { ...r, contents: nextContents } : r,
      ),
    );
    setSelected((prev) =>
      prev && prev.id === selected.id
        ? { ...prev, contents: nextContents }
        : prev,
    );
  };

  const applyRevisedTrainingJson = (trainingJson: unknown) => {
    const updatedContents = parseTrainingJson(trainingJson);
    if (!selected) return;

    setWorkingContents(cloneTrainingContents(updatedContents));
    setRows((prev) =>
      prev.map((row) =>
        row.id === selected.id ? { ...row, contents: updatedContents } : row,
      ),
    );
    setSelected((prev) =>
      prev && prev.id === selected.id
        ? { ...prev, contents: updatedContents }
        : prev,
    );
  };

  const handleRegenerate = async () => {
    if (!selected || !user?.id || !isAdmin) return;
    const critique = critiqueText.trim();
    if (!critique) {
      setRevisionError("Enter a critique before regenerating.");
      return;
    }
    ...
    try {
      ...
      const body = await response.json();
      if (!response.ok) {
        throw new Error(body.detail || "Failed to regenerate training.");
      }

      applyRevisedTrainingJson(body.revised_training_json);
      setRevisionSuccess("Revision generated.");
      setCritiqueText("");
      ...
      if (revisionsResponse.ok) {
        setRevisions((revisionsBody.revisions ?? []) as TrainingRevision[]);
      }
    } catch (err) {
      setRevisionError(
        err instanceof Error ? err.message : "Failed to regenerate training.",
      );
    } finally {
  ...
  const handleAcceptRevision = async (revisionNumber: number) => {
    if (!selected || !user?.id || !isAdmin) return;

    setAcceptLoadingRevision(revisionNumber);
    setRevisionError(null);
    setRevisionSuccess(null);
    ...
    try {
      ...
      const body = await response.json();
      if (!response.ok) {
        throw new Error(body.detail || "Failed to accept revision.");
      }

      applyRevisedTrainingJson(body.training?.training_json);
      ...
      setRevisionError(
        err instanceof Error ? err.message : "Failed to accept revision.",
      );
    } finally {
      setAcceptLoadingRevision(null);
    }
  };

  const handleRejectRevision = async (revisionNumber: number) => {
    if (!selected || !isAdmin) return;

    setRejectLoadingRevision(revisionNumber);
    setRevisionError(null);
    setRevisionSuccess(null);

    try {
      setRevisions((prev) =>
        prev.filter((revision) => revision.revision_number !== revisionNumber),
      );
      setRevisionSuccess(`Rejected revision ${revisionNumber}.`);
    } catch (err) {
      setRevisionError(
        err instanceof Error ? err.message : "Failed to reject revision.",
      );
    } finally {
      setRejectLoadingRevision(null);
    }
  };
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1 capitalize">
                    {selected.role} · Created {selected.createdAt}
                  </p>
                </div>
              ...
              {selected.status === "Draft" && (
                <div className="flex items-center justify-between">
                  <p className="text-sm text-gray-600">
                    This training is a draft. Submit it for review to publish.
                  </p>
                  <button
                ...
                <div>
                  <div className="flex items-center justify-between">
                    <p className="text-sm text-amber-700 font-medium">
                      This training is awaiting review. Approve or reject it.
                    </p>
              ...
              {selected.status === "Published" && (
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm text-green-700 font-medium">
                    This training is published and visible to employees.
                  </p>
                </div>
              )}
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm text-red-700">
                      This training was rejected. You can resubmit it for
                      review.
                    </p>
                  </div>
                  <button
              ...
              <div className="mb-6 space-y-4 rounded-xl border border-gray-200 bg-white p-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-600">
                    Add critique for the current training draft and generate a
                    revised version.
                  </p>
                </div>
                ...
                <textarea
                  ...
                  rows={5}
                  placeholder="What should change in this training draft?"
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-lg bg-[#1e3a5f] px-4 py-2 text-sm font-medium text-white hover:bg-[#152d4a] disabled:opacity-50"
                  >
                    {regenerateLoading ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
                    ) : (
              ...
              <div className="mb-6 rounded-xl border border-gray-200 bg-white p-4">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-600">
                      Latest revisions created from admin critique.
                    </p>
                  </div>
                  {loadingRevisions && (
                  ...
                  {revisions.map((revision) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        ...
                        {!revision.accepted && isAdmin && (
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() =>
                                void handleAcceptRevision(
                                  revision.revision_number,
                                )
                              }
                              disabled={
                                acceptLoadingRevision ===
                                revision.revision_number
                              }
                              className="inline-flex items-center gap-2 rounded-lg border border-[#1e3a5f] px-3 py-2 text-xs font-medium text-[#1e3a5f] hover:bg-[#1e3a5f] hover:text-white disabled:opacity-50"
                            >
                              {acceptLoadingRevision ===
                              revision.revision_number ? (
                                <Loader2 className="w-4 h-4 animate-spin" />
                            ...
                            <button
                              onClick={() =>
                                void handleRejectRevision(
                                  revision.revision_number,
                                )
                              }
                              disabled={
                                rejectLoadingRevision ===
                                revision.revision_number
                              }
                              className="inline-flex items-center gap-2 rounded-lg border border-red-300 px-3 py-2 text-xs font-medium text-red-700 hover:bg-red-50 disabled:opacity-50"
                            >
                              {rejectLoadingRevision ===
                              revision.revision_number ? (
                                <Loader2 className="w-4 h-4 animate-spin" />
      ...
      <div className="ml-64">
        <main className="p-8">
          ...
          <p className="text-sm text-gray-600 mb-6">
            {total} training{total !== 1 ? "s" : ""} available — click a row to
            study
          </p>
                  ...
                  {!loading && !error && rows.length === 0 && (
                    <tr>
                      <td
                        ...
                        className="px-6 py-6 text-sm text-gray-500"
                      >
                        No trainings found. Run the AI pipeline to generate
                        some.
                      </td>
                    </tr>
                  )}
                    ...
                    rows.map((row) => (
                      <tr
                        ...
                        onClick={() => {
                          ...
                          setSelected(row);
                          setWorkingContents(
                            cloneTrainingContents(row.contents),
                          );
                          setFlashcardSaveMsg(null);
                        }}
