    ...
    <>
      ...
      >
        <td className="px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-[#1e3a5f] flex items-center justify-center text-white text-xs font-semibold flex-shrink-0">
              {ep.employee.first_name[0]}
              {ep.employee.last_name[0]}
            </div>
            <div>
        ...
        <td className="px-6 py-4">
          <div className="flex items-center gap-2">
            <div className="w-24 h-2 bg-gray-200 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full ${ep.completionRate >= 80 ? "bg-green-500" : ep.completionRate >= 50 ? "bg-amber-500" : "bg-red-400"}`}
        ...
        <td className="px-6 py-4">
          {ep.avgScore !== null ? (
            <span
              className={`text-sm font-semibold ${ep.avgScore >= 70 ? "text-green-600" : "text-red-500"}`}
            >
              {ep.avgScore}/100
            </span>
          ) : (
                    ...
                    <div>
                      ...
                      {ev.minRequired != null && (
                        <p className="text-xs text-gray-400">
                          Min required: {ev.minRequired}/100
                        </p>
                      )}
                    </div>
                    <span
                      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${ev.passed ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                    >
                      {ev.passed ? (
                        <>
...
function downloadProgressCsv(rows: EmployeeProgress[], orgLabel: string) {
  ...
  lines.push(
    ["Employee", "TrainingId", "CompanyRole", "Score", "Passed", "CompletedAt"]
      .map(csvEscape)
      .join(","),
  );
  for (const ep of rows) {
...
export function ProgressDashboardPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setLoading(true);
      const [{ data: empData }, { data: assignData }, { data: evData }] =
        await Promise.all([
          supabase
            .from("profiles")
          ...
          supabase
            .from("assignments")
            .select("user_id, training_id, min_score")
            .eq("organization_id", organizationId),
          supabase
            .from("training_evidence")
            .select(
              "user_id, training_id, company_role, score, passed, completed_at, retake_count",
            )
            .eq("organization_id", organizationId),
        ]);
      ...
      const allTrainingIds = [
        ...new Set(rawEvidence.map((e) => e.training_id).filter(Boolean)),
      ];
      ...
      const result: EmployeeProgress[] = employees.map((emp) => {
        ...
        // Only include evidence for assigned trainings
        const empEv = evidence.filter(
          (e) => e.user_id === emp.id && assignedIds.has(e.training_id),
        );

        // Deduplicate: keep only latest try per training
        const latestByTraining: Record<number, Evidence> = {};
        empEv.forEach((e) => {
          const existing = latestByTraining[e.training_id];
          if (
            !existing ||
            new Date(e.completed_at) > new Date(existing.completed_at)
          ) {
        ...
        const avgScore =
          scores.length > 0
            ...
            : null;
        // Only count as completed if score meets the min_score need
        ...
        const completionRate =
          empAsgn.length > 0
            ? Math.min(
                100,
                Math.round((uniqueCompleted / empAsgn.length) * 100),
              )
            : 0;
        ...
        const evidenceWithRetakes = deduped.map((e) => {
          ...
          return {
            ...e,
            retakes: retakeCount[e.training_id] ?? 0,
            minRequired: asgn?.min_score ?? null,
          };
        });

        const totalRetakes = Object.values(retakeCount).reduce(
          (sum, c) => sum + c,
          0,
        );
        return {
          employee: emp,
          assigned: empAsgn.length,
          completed: uniqueCompleted,
          passed,
          avgScore,
          completionRate,
          evidence: evidenceWithRetakes,
          totalRetakes,
        };
      });
  ...
  const filtered = progress
    .filter((ep) => {
      const name =
        `${ep.employee.first_name} ${ep.employee.last_name}`.toLowerCase();
      return (
        name.includes(search.toLowerCase()) ||
        ep.employee.role.toLowerCase().includes(search.toLowerCase())
      );
    })
    .sort((a, b) => {
  ...
  const fullyComplete = progress.filter(
    (ep) => ep.completionRate === 100,
  ).length;
  const orgAvgScore =
    progress.filter((ep) => ep.avgScore !== null).length > 0
      ? Math.round(
          progress
            .filter((ep) => ep.avgScore !== null)
            .reduce((s, ep) => s + (ep.avgScore ?? 0), 0) /
            progress.filter((ep) => ep.avgScore !== null).length,
        )
      : null;
  const overallCompletion =
    totalEmp > 0
      ? Math.round(
          progress.reduce((s, ep) => s + ep.completionRate, 0) / totalEmp,
        )
      : 0;
        ...
        <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
          <div className="px-8 py-4">
            ...
            <p className="text-sm text-gray-600 mt-1">
              Company-wide training progress and performance
            </p>
          </div>
        </header>
        ...
        <main className="p-8 space-y-6">
          ...
          <div className="grid grid-cols-4 gap-5">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              <p
                className={`text-3xl font-bold ${orgAvgScore !== null && orgAvgScore >= 70 ? "text-green-600" : "text-gray-900"}`}
              >
          ...
          <div className="grid grid-cols-3 gap-5">
            ...
            <div className="bg-green-600 rounded-xl p-5">
              ...
              <p className="text-3xl font-bold text-white">
                {totalEmp > 0
                  ? Math.round((fullyComplete / totalEmp) * 100)
                  : 0}
                %
              </p>
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200">
                ...
                {!loading &&
                  filtered.map((ep) => <EmpRow key={ep.employee.id} ep={ep} />)}
              </tbody>
            </table>
          </div>
        </main>
