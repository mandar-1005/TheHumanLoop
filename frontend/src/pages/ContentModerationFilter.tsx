...
export class ContentModerationFilter {
    ...
    private static readonly HARMFUL_PATTERNS = [
        ...
        // Hacking / system exploitation — any phrasing, not just "how to"
        /\b(hack|exploit|breach|compromise)\s+(the\s+)?(system|server|database|network|app|application|account)/gi,
        // Credential/data theft
        /\b(steal|exfiltrate|dump|harvest|grab|scrape)\s+(the\s+)?(credentials|passwords|tokens|keys|data|database)/gi,
        // Offensive injection techniques
        /\b(inject|drop\s+table|sql\s*injection|xss|cross.site)\b/gi,
        // Generic illegal how-to (broadened — "ways to hack", "steps to steal", bare "phish" etc.)
        /\b(how to |ways? to |steps? to )?(hack|steal|cheat|fraud|scam|phish|spoof)\b/gi,
    ];
