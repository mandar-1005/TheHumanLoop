...
export function AIGradingChat({ training }: { training: TrainingModule }) {
    ...
    const deriveAssessment = (): Assessment => {
        const firstContent = training.contents[0];
        const firstQuestion = firstContent?.assessment?.questions?.[0];
        const prompt = firstQuestion?.prompt ?? firstQuestion?.scenario ?? 'Complete the assessment for this training module.';
        const rubricText = firstQuestion?.rubric ?? firstQuestion?.grading_rubric ?? '';
        const keyPoints = rubricText
            .match(/\*\*?([\w\s&\/]+)\*\*?\s*[\(:]/g)
            ?.map(m => m.replace(/\*\*/g, '').replace(/[\(:]/g, '').trim())
            .filter(Boolean)
            .slice(0, 5) ?? ['Review the study guide', 'Apply key concepts', 'Provide specific examples'];
        return {
            id: String(training.id),
            question: prompt,
            userAnswer: '',
            expectedKeyPoints: keyPoints,
            rubric: keyPoints.map(k => ({ criteria: k, points: Math.floor(100 / keyPoints.length) })),
        };
    };

    const [currentAssessment, setCurrentAssessment] = useState<Assessment>(() => deriveAssessment());
    ...
    const handleRegenerateGrade = () => {
        ...
        setTimeout(() => {
            const {finalScore } = analyzeAnswer(currentAssessment.userAnswer);
